...
### Файлы
 - Вырезаны `bat` файлы.  
 - Для `dev` окружения добавлен файл `app/reset` для удаления локальных файлов сгенерированных через команду `init`
 - Все тесты адаптированы под работу с сервисами и репозиториями.
 - Добавлена "прослойка" родительских abstract class.
